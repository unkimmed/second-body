...
function WebBottomTabNavigator({ initialRouteName, children, screenOptions }: any) {
        ...
        <View style={[styles.tabBarWrapper, { paddingBottom: Math.max(insets.bottom + 12, 20) }]}>
          <View style={styles.tabBar}>
            {displayTabs.map((route: any) => {
              ...
              const { name, size, label } = getIcon(route.name, isCenter, isHome, isFocused)
...
const createWebBottomTabs = createNavigatorFactory(WebBottomTabNavigator)
const { Navigator } = createWebBottomTabs()
